...
resource "aws_cloudfront_distribution" "website" { # tfsec:ignore:AWS045
  ...
  default_cache_behavior {
    ...
    response_headers_policy_id = aws_cloudfront_response_headers_policy.origin-header.id

    dynamic "function_association" {
      for_each = var.cloudfront_response_header_policy.force_origin_header ? [1] : [] # Workaround to apply condition to blocks

      content {
        event_type   = "viewer-request"
        function_arn = aws_cloudfront_function.origin-header[0].arn
      }
    }
  }
  ...
  origin {
    ...
    origin_path = var.website_origin_path
    s3_origin_config {
...
resource "aws_route53_record" "website_cloudfront_record" {
  ...
  count = var.create_website_cloudfront_record ? 1 : 0

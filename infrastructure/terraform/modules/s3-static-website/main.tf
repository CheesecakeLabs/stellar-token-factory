...
locals {
  ...
  allowed_origins         = var.website_cors_allow_all_origins ? ["*"] : concat(["http://${var.website_domain_name}", "https://${var.website_domain_name}"], var.website_cors_additional_allowed_origins)
}
...
resource "aws_s3_bucket" "log_bucket" {
  ...
  bucket = "${var.website_domain_name}-log-bucket"
...
resource "aws_route53_record" "acm_certificate_validation_records" {
  ...
  for_each = {
    for dvo in try(aws_acm_certificate.cert[0].domain_validation_options, {}) : dvo.domain_name => {

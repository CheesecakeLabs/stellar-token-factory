...
resource "aws_iam_role" "this" {
  ...
  name               = "${var.name}-ec2-rds-scheduler-lambda"
...
resource "aws_iam_role_policy" "autoscaling_group_scheduler" {
  count  = var.custom_iam_role_arn == null ? 1 : 0
  name   = "${var.name}-autoscaling-custom-policy-ec2-rds-scheduler"
...
resource "aws_iam_role_policy" "lambda_logging" {
  count  = var.custom_iam_role_arn == null ? 1 : 0
  name   = "${var.name}-lambda-ec2-rds-scheduler-logging"
  role   = aws_iam_role.this[0].id
...
resource "aws_cloudwatch_event_rule" "this" {
  ...
  name                = "${var.name}-trigger-lambda-ec2-rds-scheduler-${each.value.name}"
...
resource "aws_cloudwatch_event_target" "this" {
  ...
  rule     = "${var.name}-trigger-lambda-ec2-rds-scheduler-${each.value.name}"
...
resource "aws_cloudwatch_log_group" "this" {
  name              = "/aws/lambda/${var.name}-ec2-rds-scheduler"

...
import { defaultTransfer, transferErrors } from './constants'

export const validateInputError = (
  inputs: typeof defaultTransfer,
  setInputsErrors: Dispatch<SetStateAction<typeof transferErrors>>,
  ...
  try {
    ...
    if (!inputs.recipient) {
      ...
      setInputsErrors(values => ({
        ...values,
        ['recipient']: 'The recipient address must be informed',
      }))
    }
  } catch (e) {
    hasError = true
...
export const handleSubmitErrors = (
  responseError: typeof transferErrors,
  code: number,
  error: string,
  setInputsErrors: Dispatch<SetStateAction<typeof transferErrors>>,
  setError: Dispatch<SetStateAction<string>>
  ...
  if (responseError?.target) {
    setInputsErrors(values => ({
      ...values,
      ['recipient']: responseError.distributor,
    }))
  }
  if (!error) {

...
from .mocks.constants import SET_OPTIONS_FAIL_RESPONSES


def post_set_options_request(client: Client, network: str = None, **kwargs) -> Response:
    """
    ...
    """
    return client.post(
        path=reverse("exchange:set-options"),
...
def test_set_options_successfully(
    mocker: MockerFixture, client: Client, network: str, signers: list[str]
) -> None:
    request_data = {
    ...
    load_account_mock = mocker.patch(
        ...
        side_effect=[
            get_mocked_account_object(
                account_id=request_data["public_key"],
            ),
        ],
    )

    response = post_set_options_request(client=client, network=network, **request_data)
...
@pytest.mark.parametrize("request_data,error", SET_OPTIONS_FAIL_RESPONSES)
def test_set_options_fails_when_request_data_is_wrong(
    client: Client, request_data: dict, error: dict
):
    response = post_set_options_request(client, **request_data)

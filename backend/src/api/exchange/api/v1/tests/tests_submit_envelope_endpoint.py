...
def post_submit_envelope_request(
    ...
    """
    return client.post(
        path=reverse("exchange:submit-envelope"),
...
def test_submit_envelope_successfully(
    ...
    response = post_submit_envelope_request(
        client=client, network=network, envelope_xdr=ENVELOPE_XDR
    )
...
def test_submit_invalid_envelope(
    ...
    response = post_submit_envelope_request(
        client=client, envelope_xdr="invalid-envelope"
    )
...
def test_submit_envelope_with_stellar_error(
    ...
    response = post_submit_envelope_request(client=client, envelope_xdr=ENVELOPE_XDR)

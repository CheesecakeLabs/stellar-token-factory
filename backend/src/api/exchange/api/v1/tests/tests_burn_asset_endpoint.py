...
from .mocks.constants import BURN_ASSET_FAIL_RESPONSES


def post_burn_asset_request(client: Client, network: str = None, **kwargs) -> Response:
    """
    Make a request to burn asset
    Args:
        client: HTTP Client
    """
    return client.post(
        path=reverse("exchange:burn-asset"),
...
def test_burn_asset_successfully(
    ...
    response = post_burn_asset_request(client=client, network=network, **request_data)
...
@pytest.mark.parametrize("request_data,error", BURN_ASSET_FAIL_RESPONSES)
def test_burn_asset_fails_when_request_data_is_wrong(
    client: Client, request_data: dict, error: dict
):
    response = post_burn_asset_request(client, **request_data)

...
def post_clear_auth_flag_request(
    client: Client, network: str = None, **kwargs
) -> Response:
    """
    Make a request to clear auth flag
    Args:
        client: HTTP Client
    """
    return client.post(
        path=reverse("exchange:clear-auth-flag"),
...
def test_clear_auth_flag_successfully(
    ...
    response = post_clear_auth_flag_request(
...
def test_clear_auth_flag_fails_when_request_data_is_wrong(
    client: Client, request_data: dict, error: dict
):
    response = post_clear_auth_flag_request(client, **request_data)

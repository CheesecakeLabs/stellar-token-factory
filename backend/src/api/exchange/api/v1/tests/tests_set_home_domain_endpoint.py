...
from .mocks.constants import SET_HOME_DOMAIN_FAIL_RESPONSES


def post_set_home_domain_request(
    client: Client, network: str = None, **kwargs
) -> Response:
    """
    Make a request to set home domain
    Args:
        client: HTTP Client
    """
    return client.post(
        path=reverse("exchange:set-home-domain"),
...
def test_set_home_domain_successfully(
    ...
    response = post_set_home_domain_request(
...
@pytest.mark.parametrize("request_data,error", SET_HOME_DOMAIN_FAIL_RESPONSES)
def test_set_home_domain_fails_when_request_data_is_wrong(
    client: Client, request_data: dict, error: dict
):
    response = post_set_home_domain_request(client, **request_data)

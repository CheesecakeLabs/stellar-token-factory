...
def get_payees_list_request(
    client: Client,
) -> Response:
    """
    Make a request to get payees list
    Args:
        client: HTTP Client
    """
    return client.get(
        path=reverse(
            "exchange:get-payees-list",
        ),
...
def test_get_payees_list_successfully(
    client: Client,
) -> None:

    response = get_payees_list_request(client=client)

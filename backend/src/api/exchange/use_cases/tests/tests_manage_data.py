...
from api.exchange.use_cases import ManageDataUseCase
...
def test_manage_data_fails_when_public_key_is_invalid():
    data = {
        "network": "TESTNET",
    ...
    with pytest.raises(
        ...
        ManageDataUseCase().execute(**data)
...
def test_manage_data_fails_when_account_not_found(
...
@pytest.mark.parametrize(
    "network,name,value",
    (
        ("TESTNET", "Data name", "Data value"),
        ("PUBLIC", "Data name very very very very very very very very long", None),
    ),
)
def test_manage_data_succesfully(
    mocker: MockerFixture, network: str, name: str, value: str
):
    data = {
        "network": network,
        "public_key": Keypair().public_key,
    ...
    response: dict = ManageDataUseCase().execute(**data)
    ...
    assert type(envelope.transaction.operations[0]) == ManageData
    assert envelope.transaction.operations[0].source.account_id == data["public_key"]
    assert envelope.transaction.operations[0].data_name == data["name"]
    assert envelope.transaction.operations[0].data_value == (
        data["value"].encode() if data["value"] else None
    )
